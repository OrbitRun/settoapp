...
- Migration 1 — harden `claim_group_invitation`: for a person invitation, in the same transaction as the person link, clear `removed_at` on the existing `group_members` row (insert one only when none exists), keeping role and default split fields. Returns `claimed` + group id as today. Group-wide invitations unchanged.
...
- Receipt storage order in `src/lib/account.functions.ts` stays: cleanup RPC → read own `storage_path` values → delete objects → verify → admin delete user. Unchanged apart from being covered by tests.
