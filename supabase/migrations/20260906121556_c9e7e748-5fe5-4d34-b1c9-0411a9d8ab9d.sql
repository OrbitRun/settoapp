...
BEGIN
  ...
  -- It never reconstructs one, active or former.
  ...
  -- Already claimed by this same account: idempotent success, but make sure
  -- the membership really is active before reporting it.
  IF v_person.linked_profile_id = auth.uid() THEN
    UPDATE public.group_members gm
    SET removed_at = NULL
    WHERE gm.id = v_member.id AND gm.removed_at IS NOT NULL;
  ...
  -- Link the exact historical person and reactivate its existing membership,
  -- atomically. Role and default split fields are preserved as they were.
...
BEGIN
  ...
  FOREACH v_group_id IN ARRAY v_owned_groups LOOP
    ...
    ELSE
      -- Nobody else has a real account: no shared history to preserve, so the
      -- group and everything chained to it goes away with the account.
