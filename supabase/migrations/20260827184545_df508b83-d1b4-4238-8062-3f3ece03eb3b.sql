CREATE OR REPLACE FUNCTION public.create_group(_name text, _default_split_type text, _person_names text[] DEFAULT '{}'::text[], _percentages jsonb DEFAULT '{}'::jsonb, _shares jsonb DEFAULT '{}'::jsonb, _person_ids uuid[] DEFAULT '{}'::uuid[])
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  n := COALESCE(array_length(_person_names, 1), 0);
  FOR i IN 1 .. n LOOP
    v_raw := _person_names[i];
    v_trimmed := btrim(COALESCE(v_raw, ''));
    CONTINUE WHEN v_trimmed = '';
    v_key := lower(v_trimmed);

    v_hint := NULL;
    IF _person_ids IS NOT NULL AND array_length(_person_ids, 1) >= i THEN
      v_hint := _person_ids[i];
    END IF;

    v_person_id := NULL;

    -- Durable identity wins: when the caller supplies the exact person the
    -- split already references, reuse that row instead of guessing by name.
